...
namespace Tools
{
	[Serializable] public abstract class LNode
	{
		[NonSerialized] public TokensGen m_tks;
		public int m_state;
	...
	[Serializable] public class TokClassDef
	{
	...
	[Serializable] public class Dfa : LNode
	{
		...
		[NonSerialized] Tokens m_tokens = null;
		...
		[NonSerialized] internal NList m_nfa = new NList(); // nfa nodes in m_state order
		...
		*/
		public Regex(TokensGen tks,string str) 
		{
			...
			{ // identify a bracketed expression
				...
				if (j==n)
					goto bad;
				m_sub = new Regex (tks,str.Substring(1,j-1));
				j++;
				//3.  Look to see if the string begins with a bracket [ . If so, find the matching ] , watching for escapes.
				//	Construct a ReRange for everything between the []s. 
				//	Mark everything up to the ] as used, and go to step 9.
			...
			// 12.  If the next character is a | , build a ReAlt using the m_sub we have and the rest of the string.
			if (str[j]=='|')
				m_sub = new ReAlt(tks,m_sub,str.Substring(j+1,n-j-1));
				// 13.  Otherwise build a ReCat using the m_sub we have and the rest of the string.
			else if (j<n)
				m_sub = new ReCat(tks,m_sub,str.Substring(j,n-j));
			return;
			bad:
				Console.WriteLine(String.Format("ill-formed regular expression <{0}>\n", str));
			Environment.Exit(-1);
		}	
	...
	{
		public ReAlt(TokensGen tks,Regex sub,string str) 
		{
			m_sub = sub; 
			m_alt = new Regex(tks,str);
		}
		public Regex m_alt;
	...
	{
		public ReCat(TokensGen tks,Regex sub, string str) 
		{
			m_sub = sub;
			m_next = new Regex(tks,str);
		}
		Regex m_next;
		public override void Print(TextWriter s) 
		{
